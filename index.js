      ...
      const result = await axios.post('https://api.openai.com/v1/chat/completions', {
          ...
          messages: [{
            ...
            //   content: "Provide a summary of the RFP document in JSON format with keys for 'objective', 'description', 'value', 'duedate' and 'questiondate': " + textExtracted
              content: "Provide a summary of the RFP document stricly in JSON format with and only for the keys of 'title','department','type','event_id','objective', 'description', 'date_of_release','date_of_clarification','date_of_submission','qualification_criteria','contract_term' and 'point_of_contact'. If there is no valid value for the respective key then return the value as not defined for that key. Stricly do not give any other keys in response other than which is provided in the prompt. Give the information accurate enough in detail without missing out on any important information. If required provide the summaru splitted in points format inside the value of the respected json keys but make sure no important information is missed out since the information you provide should be in accurate from the information provided." + first5000Words
